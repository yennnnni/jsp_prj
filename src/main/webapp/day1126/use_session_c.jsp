  ...
  ================================================== -->
		...
		<div class="container marketing">
         ...
         document.addEventListener("DOMContentLoaded", function () {
			...
			<%
			//이전 페이지에 존재하는 값을 사용하기 위해서 session을 사용한다
			String name=(String)session.getAttribute("name");
			Date date=(Date)session.getAttribute("connectTime");
			//1. 직접 호출하면 세션에 설정된 값이 없으므로 null이 나온다.
			//( 비정상적인 요청 => 페이지 이동 )
			//2. 설정한 interval 시간내에 요청이 없으면 세션의 값이 삭제된다.( null 반환 )
			//정상적인 요청이었으나 오랜시간 자리 비움 ( 다른 사람일수 있음 )
			if( name == null ){
